...
export default function LabConsole({ lab }: { lab: LabMeta }) {
  ...
  return (
    <div className="flex h-full flex-col rounded-lg border border-divider bg-bg-base">
      <div className="flex items-center gap-2 border-b border-divider px-3 py-1.5 text-[calc(11px_*_var(--text-scale))] text-ink-dim">
        ...
        <div className="flex flex-wrap gap-1.5 border-b border-divider px-3 py-2">
          {lab.sidecar_cmds.map((c) => (
            <button
              ...
              className="rounded-md border border-divider bg-bg-surface px-2 py-0.5 font-mono text-[calc(11px_*_var(--text-scale))] text-ink-muted hover:border-accent hover:text-accent"
            >
              {c}
            </button>
          ))}
      ...
      {/* Scrollback. */}
      <div className="min-h-0 flex-1 overflow-auto px-3 py-2 font-mono text-[calc(12px_*_var(--text-scale))] leading-relaxed">
        {log.length === 0 ? (
      ...
      <div className="flex items-center gap-2 border-t border-divider px-3 py-2">
        ...
        <input
          ...
          disabled={busy}
          className="flex-1 bg-transparent font-mono text-[calc(12px_*_var(--text-scale))] text-ink-primary outline-none placeholder:text-ink-dim"
        />
        <button
          onClick={() => run()}
          disabled={busy}
          className="rounded-md bg-accent px-2.5 py-1 text-[calc(11px_*_var(--text-scale))] font-semibold text-bg-base disabled:opacity-50"
        >
          Run
        </button>
      </div>
